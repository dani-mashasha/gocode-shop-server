...
const ProductsBar = () =>{
   ...
   const FillterAll = (range, category) => {
    let fillterdProducts;
      category === "all items"? fillterdProducts = productsOrigin : fillterdProducts = productsOrigin.filter((p)=>p.category === category);
  
     if(range){
      fillterdProducts = fillterdProducts.filter(product => product.price >= range[0] && product.price <= range[1]);
     }
